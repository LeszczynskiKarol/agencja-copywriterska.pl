...
BUCKET_NAME="agencja-copywriterska-attachments"
PRESIGN_FUNCTION="agencja-copywriterska-presign"
CONTACT_FUNCTION="agencja-copywriterska-contact"
API_NAME="agencja-copywriterska-api"
ROLE_NAME="agencja-copywriterska-lambda-role"
DOMAIN="agencja-copywriterska.pl"

# Katalog tymczasowy kompatybilny z Windows
...
if [ -n "$EXISTING_API_ID" ] && [ "$EXISTING_API_ID" != "None" ]; then
  ...
  API_ID=$(aws apigatewayv2 create-api \
    --name "$API_NAME" \
    --protocol-type "HTTP" \
    --cors-configuration '{"AllowOrigins":["https://agencja-copywriterska.pl","http://localhost:4321"],"AllowMethods":["POST","OPTIONS"],"AllowHeaders":["Content-Type"],"MaxAge":3600}' \
    --region "$REGION" \
